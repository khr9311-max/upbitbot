...
| `python backtest.py --markets KRW-BTC --days 365` | 워크포워드 백테스트 (고정 종목) |
| `python backtest.py --dynamic-universe --days 365` | 자동선정까지 재현한 백테스트 |
...
- **최소 상장 경과일** ≥ `UNIVERSE_MIN_LISTING_DAYS` (기본 14일) — 신규상장 직후
  거래대금이 일시 폭증했다가 급락하는 "상장빨" 코인이 상위권에 잡히는 것을 차단
...
**알트코인도 이미 대상입니다.** 스크리너는 KRW 마켓 288종목(2026-08 기준) 전체에서
필터를 통과한 종목을 자동으로 뽑습니다. `UNIVERSE_MIN_TRADE_PRICE_24H` 를 낮출수록
DOGE·SUI·ONDO·XLM 같은 검증된 중견 알트가 후보에 들어옵니다.

```bash
python main.py universe    # 지금 어떤 종목이 선정되는지 바로 확인
```
...
python backtest.py --markets KRW-BTC,KRW-ETH,KRW-XRP --days 365 --end 2025-08-27   # 과거 특정 구간만 자르기
```

리포트는 총수익률 / CAGR / MDD / 샤프 / 승률 / 손익비 / 손익팩터 / 켈리 f\* /
**단순보유 대비 초과성과**와 전략별·국면별 분해를 출력하고, 자산 곡선을
`data/backtest_equity.csv` 로 저장합니다. 캔들은 `data/candles/` 에 캐시되므로
두 번째 실행부터는 API 호출 없이 즉시 돌아갑니다.

### 동적 유니버스 백테스트 (`--dynamic-universe`)

위 명령들은 `--markets` 로 지정한 고정 종목만 검증합니다. 실거래 봇은 6시간마다
자동으로 종목을 갈아타므로, **자동선정 자체를 검증**하려면 이 플래그를 씁니다.

```bash
python backtest.py --dynamic-universe --days 365 --end 2025-08-27 --seed 300000
```

KRW 마켓 전 종목의 일봉으로 과거 시점의 24시간 거래대금을 복원해, 실거래
스크리너와 동일한 필터(유의/주의/스테이블코인/신규상장 제외, 거래대금 하한)를
6시간마다 재현합니다. 다만 두 가지는 과거 이력이 없어 재현하지 못합니다:

- **호가 스프레드 필터** — 과거 호가창 데이터가 없어 생략됩니다.
- **유의/주의 종목 플래그** — 업비트 API 가 이력을 제공하지 않아 **현재 시점의
  플래그를 전체 기간에 일괄 적용**합니다. 지금 유의종목인 코인은 검증 기간
  내내 제외되고, 그 반대(과거엔 유의종목이었지만 지금은 아닌 경우)는 잡아내지
  못합니다.

첫 실행은 288개 종목의 일봉을 수집하느라 몇 분 걸리지만, 이후 실행부터는 캐시로
즉시 끝납니다.
...
### 하락장 구간 (2025-08-27 ~ 2026-08-28, 366일)

| | 수익률 | 최대낙폭 |
|---|---|---|
| KRW-BTC 단순보유 | −29.19% | −50.85% |
| KRW-ETH 단순보유 | −45.61% | −65.88% |
| KRW-XRP 단순보유 | −52.22% | −68.36% |
| **3종목 균등분산 단순보유** | **−42.81%** | −68.36% |
| **봇 (기본 설정)** | **−4.48%** | **−8.15%** |
| **봇 (그리드 단독)** | **−0.19%** | **−3.34%** |

**단순보유 대비 초과성과 +38.34%p, 최대낙폭 −68% → −8%.**

절대 수익률만 보면 −4.48% 는 실패로 보입니다. 하지만 이 구간은 BTC 가 1억 5,500만원에서
1억 1,000만원으로 빠진 극심한 하락장이었고, 검증 봉의 **49.7% 가 STRONG_BEAR** 였습니다.
그냥 들고 있었으면 42% 를 잃을 자리에서 봇은 4.5% 만 잃었습니다.
국면 분류기가 하락 국면 자본 배분을 0% 로 유지하며 제 역할을 한 결과입니다.

> **봇 수익률은 절대값이 아니라 단순보유 대비로 읽어야 합니다.**
> 하락장의 −5% 는 훌륭한 성과이고 상승장의 −5% 는 실패입니다.
> 백테스터가 리포트에 `단순보유 대비` 줄을 항상 출력하는 이유입니다.

### 다른 구간도 반드시 직접 검증하세요

```bash
# 과거 특정 구간(예: 상승장)을 잘라서 검증
python backtest.py --markets KRW-BTC,KRW-ETH,KRW-XRP --days 365 --end 2025-08-27
python backtest.py --markets KRW-BTC --days 730
```

한 구간의 백테스트는 증거가 아니라 반증 도구입니다. 하락장 방어를 확인했다면
**상승장에서 상승분을 얼마나 따라가는지(업사이드 캡처)** 를 반드시 별도로 확인하세요.
방어만 잘하고 상승을 놓치는 봇은 장기적으로 단순보유를 이기지 못합니다.

### 전략별 분해와 해석 (하락장 구간 기준)

그리드 −3,393원 / 추세 −8,825원 / DCA −1,218원

- **DCA 승률 8~10% 는 전략서 설계 그대로의 결과입니다.** 48시간 타임스톱이 하락 추세에서
  반등을 기다리지 않고 손실을 확정하기 때문입니다. 자본 보호에는 유리하지만
  하락장이 길면 잔손실이 누적됩니다.
- **추세 전략은 하락장에서 손실을 내는 게 정상입니다.** 상승 국면이 15.9% 뿐이었고
  그마저 대부분 하락 추세 중 반등이었습니다. 이 전략의 값어치는 상승장 구간에서 나옵니다.
- 성과가 안 나오는 구성은 **국면 배분 비중을 0으로 꺼서** 검증할 수 있습니다
